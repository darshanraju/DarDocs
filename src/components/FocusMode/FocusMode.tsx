...
export function FocusMode({ isActive, onExit, children }: FocusModeProps) {
  ...
  const handleBackdropClick = useCallback(
    (e: React.MouseEvent) => {
      // Only exit if clicking the very edge (not the content area)
      if (e.target === e.currentTarget) {
        // Don't exit, just let it be
      }
    },
    []
  );
  ...
  return (
    <div className="focus-mode-overlay" onClick={handleBackdropClick}>
      {/* Top toolbar - minimal */}
      ...
      <div ref={containerRef} className="focus-mode-content">
        <div ref={contentRef} className="focus-mode-editor">
          {children}
        </div>
      </div>
    </div>
