...
import { convertMarkdownToTipTap } from '@dardocs/core';
...
export function ImportMarkdown() {
  ...
  const handleFileSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = convertMarkdownToTipTap(text);

      // Create new document with converted content
      const title = file.name.replace(/\.(md|markdown|mdown|mkd|mdx)$/i, '');
      ...
      if (result.warnings.length > 0) {
        ...
        console.warn('Markdown Import warnings:', result.warnings);
      } else {
  ...
  return (
    <>
      <input
        ref={fileInputRef}
        type="file"
        accept={ACCEPTED_FILE_TYPES.markdown}
        onChange={handleFileSelect}
        className="hidden"
      />
      <Tooltip content="Import Markdown file">
